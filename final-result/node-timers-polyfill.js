...
export function setTimeout(after, value, options = {}) {
  ...
  if (typeof ref !== 'boolean') return Promise.reject(new ERR_INVALID_ARG_TYPE('options.ref', 'boolean'))
  if (signal?.aborted) return Promise.reject(new AbortError())
  let oncancel
  const ret = new Promise((resolve, reject) => {
    const timeout = _setTimeout(resolve, after, value)
    if (typeof timeout?.unref === 'function' && !ref) timeout.unref()
    if (signal) {
      oncancel = cancelListenerHandler.bind(undefined, timeout, clearTimeout, reject)
      signal.addEventListener('abort', oncancel, { once: true })
    }
  })
  return oncancel !== undefined ? ret.finally(() => signal.removeEventListener('abort', oncancel)) : ret
}
