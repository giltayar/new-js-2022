...
await Promise.all([
  setTimeout(1000).then(() => Promise.reject(1)),
  setTimeout(4000).then(() => console.log("done")),
]).catch(() => console.log("promise.all done"))
...
await Promise.allSettled([
  ...
  setTimeout(4000).then(() => console.log("done")),
]).then(() => console.log("promise.allSettled done"))
...
await Promise.allSettled([
  setTimeout(1000)
    .then(() => Promise.reject(1))
    .finally(() => ac.abort()),
  setTimeout(4000, null, { signal: ac.signal }).finally(() => console.log("done")),
]).then(() => console.log("promise.allSettled done"))
